...
int grading(char stud_answer[1000],char test_answer[1000]){
    int plus_points;
    int minus_points;
    int pm_points=0;
    ...
    if(num_of_answers(test_answer)!=0){
        ...
        // Checking if any correct answers are missing, if yes negative points are given
        for(int i=7;i>-1;i--){
            if(isalpha(test_answer[i])){
                bool wrong_answer=true;
                for(int j=0;stud_answer[j]!='\0';j++){
                    if(test_answer[i]==stud_answer[j]){
                        wrong_answer=false;
                        break;
                    }
                }
        ...
        if(num_of_answers(test_answer)==plus_points && minus_points==0){
            pm_points=4;
        }else if(num_of_answers(test_answer)==plus_points && minus_points!=0){
            pm_points=4;
            pm_points-=minus_points;
        }else{
            pm_points=plus_points-minus_points;
            if(pm_points>4) pm_points=3;
        }
        if(pm_points<0) return 0;
        return pm_points;
...
int main() {
    ...
    input_file_to_array(read_answers,read_answers_file);
    int sum=0;
    
    for(int i = 0; i < 10; i++) {
        int random_test=rand()%10;  //  Randomly picking exam for every student
        for(int j = 0; j < 10; j++) {
